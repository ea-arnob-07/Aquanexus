...
HudLayout makeHudLayout() {
  HudLayout h{};
  for (int i = 0; i < 3; ++i)
    h.fan[i] = {32.0f, 274.0f + i * 64.0f, 266.0f, 54.0f};
  h.allFans = {32.0f, 466.0f, 266.0f, 42.0f};
  for (int i = 0; i < 3; ++i)
    h.speed[i] = {32.0f + i * 92.0f, 596.0f, 82.0f, 44.0f};
...
void App::initWindow() {
  ...
  window_ = glfwCreateWindow(
      width_, height_, "AquaNexus - Photoreal 3-Pond Aquaculture Digital Twin",
      nullptr, nullptr);
  if (!window_) {
    glfwTerminate();
  ...
  if (e != GLEW_OK)
    throw std::runtime_error(std::string("GLEW initialization failed: ") +
                             (const char *)glewGetErrorString(e));
  glGetError();
...
void App::scrollCallback(GLFWwindow *w, double, double y) {
  if (auto *a = (App *)glfwGetWindowUserPointer(w))
    a->onScroll(y);
}
void App::framebufferCallback(GLFWwindow *w, int wi, int he) {
  if (auto *a = (App *)glfwGetWindowUserPointer(w)) {
    a->width_ = std::max(1, wi);
    a->height_ = std::max(1, he);
    if (a->scene_)
      a->scene_->resize(a->width_, a->height_);
  }
}
void App::onScroll(double y) {
  camera_.distance = clampf(camera_.distance - (float)y * 2.6f, 17.0f, 82.0f);
}
...
bool App::keyPressed(int key) const {
  return glfwGetKey(window_, key) == GLFW_PRESS && !prevKeys_[key];
}
std::string App::fnum(float v, int precision) {
...
void App::processInput(float dt) {
  ...
  if (keyPressed(GLFW_KEY_B))
    sensorHudExpanded_ = !sensorHudExpanded_;
  ...
  if (keyPressed(GLFW_KEY_C))
    camera_.setCinematic(!camera_.cinematic);
  if (keyPressed(GLFW_KEY_SPACE))
    paused_ = !paused_;
  if (keyPressed(GLFW_KEY_X))
    simSpeed_ = simSpeed_ < 2.0f ? 5.0f : (simSpeed_ < 10.0f ? 20.0f : 1.0f);
  ...
  if (click && showUI_) {
    ...
    if (pointInRect(mx, my, hud.allFans.x, hud.allFans.y, hud.allFans.w,
                    hud.allFans.h)) {
      bool all = sim_.fans()[0].on && sim_.fans()[1].on && sim_.fans()[2].on;
      sim_.setAllFans(!all);
    }
    ...
    if (pointInRect(mx, my, hud.pause.x, hud.pause.y, hud.pause.w, hud.pause.h))
      paused_ = !paused_;
    if (pointInRect(mx, my, hud.reset.x, hud.reset.y, hud.reset.w,
                    hud.reset.h)) {
      sim_.reset();
      camera_.reset();
      simSpeed_ = 1.0f;
    ...
    if (pointInRect(mx, my, hud.day.x, hud.day.y, hud.day.w, hud.day.h))
      setEnvironmentMode(false);
    if (pointInRect(mx, my, hud.night.x, hud.night.y, hud.night.w, hud.night.h))
      setEnvironmentMode(true);
    if (pointInRect(mx, my, hud.fullscreen.x, hud.fullscreen.y,
                    hud.fullscreen.w, hud.fullscreen.h))
      toggleFullscreen();
  }
  prevMouseLeft_ = ml;
  for (int k = 0; k <= GLFW_KEY_LAST; ++k)
    prevKeys_[k] = (unsigned char)(glfwGetKey(window_, k) == GLFW_PRESS);
}
...
void App::drawUI() {
  ...
  auto button = [&](const HudRect &r, const glm::vec4 &bg,
                    const glm::vec4 &edge) {
    ui_->roundedRect(r.x, r.y, r.w, r.h, 8, edge);
    ui_->roundedRect(r.x + 1, r.y + 1, r.w - 2, r.h - 2, 7, bg);
  };
  auto centered = [&](const HudRect &r, float y, float scale,
                      const std::string &label, const glm::vec4 &color) {
    ui_->text(r.x + (r.w - ui_->textWidth(scale, label)) * 0.5f, y, scale,
              label, color);
  };
  ...
  const StatusRow statusRows[3] = {
      ...
      {"CIRCULATION", circ > 0.10f ? "ACTIVE" : "IDLE",
       circ > 0.10f ? green : amber},
  ...
  for (int i = 0; i < 3; ++i) {
    const auto &f = sim_.fans()[i];
    const HudRect &r = hud.fan[i];
    glm::vec4 bg = f.on ? glm::vec4(0.018f, 0.17f, 0.13f, 0.62f)
                        : glm::vec4(0.17f, 0.040f, 0.060f, 0.62f);
    glm::vec4 edge = f.on ? glm::vec4(0.13f, 0.62f, 0.44f, 0.74f)
                          : glm::vec4(0.65f, 0.15f, 0.22f, 0.74f);
    button(r, bg, edge);
    ui_->circle(r.x + 13, r.y + 14, 4.0f, f.on ? green : red, 16);
    ui_->text(r.x + 24, r.y + 8, 0.92f,
              "FAN " + std::to_string(i + 1) + "  POND " +
                  std::to_string(f.fromPond + 1) + " -> POND " +
                  std::to_string(f.toPond + 1),
              text);
    ui_->text(r.x + 12, r.y + 31, 0.78f,
              f.on ? "RUNNING - CLICK TO STOP" : "STOPPED - CLICK TO START",
              f.on ? green : red);
    std::string flow = fnum(52.0f * f.speed, 0) + " L/S";
    ui_->text(r.x + r.w - ui_->textWidth(0.78f, flow) - 12, r.y + 31, 0.78f,
              flow, cyan);
    HudRect badge{r.x + r.w - 43, r.y + 7, 34, 16};
    ui_->roundedRect(badge.x, badge.y, badge.w, badge.h, 6,
                     {0.035f, 0.105f, 0.145f, 0.64f});
    centered(badge, badge.y + 5, 0.58f, "KEY " + std::to_string(i + 1), muted);
  }
  button(hud.allFans,
         allOn ? glm::vec4(0.20f, 0.035f, 0.055f, 0.66f)
               : glm::vec4(0.025f, 0.21f, 0.14f, 0.66f),
         allOn ? glm::vec4(0.75f, 0.16f, 0.23f, 0.78f)
               : glm::vec4(0.12f, 0.68f, 0.45f, 0.78f));
  ui_->text(hud.allFans.x + 13, hud.allFans.y + 14, 0.92f,
            allOn ? "STOP ALL CIRCULATION FANS" : "START ALL CIRCULATION FANS",
            allOn ? red : green);
  ui_->text(hud.allFans.x + hud.allFans.w - ui_->textWidth(0.72f, "F KEY") - 12,
            hud.allFans.y + 15, 0.72f, "F KEY", muted);
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    button(hud.speed[i],
           active ? glm::vec4(0.025f, 0.20f, 0.17f, 0.66f)
                  : glm::vec4(0.025f, 0.065f, 0.095f, 0.62f),
           active ? glm::vec4(0.12f, 0.72f, 0.53f, 0.78f) : panelEdge);
    centered(hud.speed[i], hud.speed[i].y + 7, 0.84f, speedNames[i],
             active ? green : text);
    centered(hud.speed[i], hud.speed[i].y + 26, 0.70f,
             std::to_string((int)speeds[i]) + "X SPEED", muted);
  }
  button(hud.pause,
         paused_ ? glm::vec4(0.20f, 0.13f, 0.025f, 0.66f)
                 : glm::vec4(0.025f, 0.065f, 0.095f, 0.62f),
         paused_ ? glm::vec4(0.72f, 0.46f, 0.10f, 0.78f) : panelEdge);
  centered(hud.pause, hud.pause.y + 7, 0.84f,
           paused_ ? "RESUME SIM" : "PAUSE SIM", paused_ ? amber : text);
  ...
  button(hud.day,
         nightMode_ ? glm::vec4(0.025f, 0.065f, 0.095f, 0.58f)
                    : glm::vec4(0.20f, 0.15f, 0.035f, 0.68f),
         nightMode_ ? panelEdge : glm::vec4(0.80f, 0.57f, 0.13f, 0.80f));
  centered(hud.day, hud.day.y + 7, 0.82f, "DAY", nightMode_ ? muted : amber);
  centered(hud.day, hud.day.y + 26, 0.64f, "D KEY", muted);
  button(hud.night,
         nightMode_ ? glm::vec4(0.025f, 0.13f, 0.20f, 0.68f)
                    : glm::vec4(0.025f, 0.065f, 0.095f, 0.58f),
         nightMode_ ? glm::vec4(0.13f, 0.55f, 0.82f, 0.80f) : panelEdge);
  centered(hud.night, hud.night.y + 7, 0.76f, "NIGHT",
           nightMode_ ? cyan : muted);
  centered(hud.night, hud.night.y + 26, 0.64f, "N KEY", muted);
  button(hud.fullscreen, {0.025f, 0.080f, 0.115f, 0.62f}, panelEdge);
  centered(hud.fullscreen, hud.fullscreen.y + 7, 0.76f,
           fullscreen_ ? "EXIT FULLSCREEN" : "ENTER FULLSCREEN", cyan);
  centered(hud.fullscreen, hud.fullscreen.y + 26, 0.68f, "F11 KEY", muted);

  // Live telemetry card on the right. B opens a larger, easier-to-read detail
  // view.
  if (width_ > 760) {
    const auto &s = sim_.sensors();
    struct Row {
      std::string n, v, detail;
      float score;
    };
    std::vector<Row> rows = {
        {"TEMPERATURE", fnum(s.temperatureC, 1) + " C",
         "LIVE POND WATER TEMPERATURE",
         1.0f - saturate(std::fabs(s.temperatureC - 28.5f) / 6.0f)},
        {"AMMONIA NH3", fnum(s.ammoniaMgL, 2) + " MG/L",
         "UNIONISED AMMONIA CONCENTRATION",
         1.0f - saturate(s.ammoniaMgL / 1.45f)},
        {"PRESSURE", fnum(s.pressureKPa, 1) + " KPA",
         "HYDROSTATIC PIPE PRESSURE", 1.0f},
        {"PH LEVEL", fnum(s.pH, 2), "WATER ACIDITY AND ALKALINITY",
         1.0f - saturate(std::fabs(s.pH - 7.4f) / 1.55f)},
        {"DISSOLVED O2", fnum(s.dissolvedOxygenMgL, 2) + " MG/L",
         "OXYGEN AVAILABLE TO THE FISH",
         saturate((s.dissolvedOxygenMgL - 3.0f) / 4.2f)},
        {"WATER LEVEL", fnum(s.waterLevelM, 2) + " M",
         "CURRENT POND WATER DEPTH",
         1.0f - saturate(std::fabs(s.waterLevelM - 1.18f) / 0.35f)}};
    glm::vec4 qc =
        s.quality01 > 0.65f ? green : (s.quality01 > 0.38f ? amber : red);
    std::string qv =
        fnum(s.quality01 * 100, 1) + "%  " +
        (s.quality01 > 0.65f ? "GOOD"
                             : (s.quality01 > 0.38f ? "WATCH" : "POOR"));
    std::string liveState = sim_.circulation01() > 0.10f
                                ? "CIRCULATION STABLE"
                                : "CIRCULATION STOPPED";

    if (!sensorHudExpanded_) {
      float rw = 324.0f, rx = (float)width_ - rw - 20.0f, ry = 82.0f,
            rh = 430.0f;
      card(rx, ry, rw, rh);
      ui_->roundedRect(rx + 14, ry + 15, 34, 3, 1.5f, cyan);
      ui_->text(rx + 58, ry + 7, 1.45f, "POND 2 TELEMETRY", text);
      ui_->text(rx + 14, ry + 35, 0.92f, "LIVE SENSOR NETWORK - 6 CHANNELS",
                muted);
      float yy = ry + 67;
      for (size_t i = 0; i < rows.size(); ++i) {
        float score = rows[i].score;
        glm::vec4 status =
            score > 0.66f ? green : (score > 0.38f ? amber : red);
        HudRect row{rx + 14, yy + (float)i * 45.0f, rw - 28, 37};
        ui_->roundedRect(row.x, row.y, row.w, row.h, 7,
                         {0.018f, 0.060f, 0.078f, 0.58f});
      ...
      ui_->roundedRect(rx + 14, qy + 21, rw - 28, 12, 6,
                       {0.015f, 0.050f, 0.065f, 0.62f});
      ...
      ui_->text(rx + rw - lw - 14, qy + 45, 0.82f, liveState,
                sim_.circulation01() > 0.10f ? cyan : amber);
      ...
      ui_->text(rx + 22, ry + 46, 1.04f, "LIVE SENSOR NETWORK - 6 CHANNELS",
                muted);
      ...
      for (size_t i = 0; i < rows.size(); ++i) {
        ...
        std::string statusText =
            score > 0.66f ? "NORMAL" : (score > 0.38f ? "WATCH" : "ALERT");
        HudRect row{rx + 22, yy + (float)i * rowStep, rw - 44, rowHeight};
        ui_->roundedRect(row.x, row.y, row.w, row.h, 9,
                         {0.018f, 0.060f, 0.078f, 0.62f});
      ...
      ui_->text(rx + rw - lw - 22, qy + 65, 1.04f, liveState,
                sim_.circulation01() > 0.10f ? cyan : amber);
      ui_->text(rx + 22, qy + 94, 0.92f,
                "VALUES UPDATE LIVE WITH DAY NIGHT AND CIRCULATION CONDITIONS",
                muted);
    }
  }
...
void App::run() {
#ifdef __EMSCRIPTEN__
  emscripten_set_main_loop_arg([](void *p) { static_cast<App *>(p)->frame(); },
                               this, 0, 1);
#else
